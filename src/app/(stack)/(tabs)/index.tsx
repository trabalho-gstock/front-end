    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "http://192.168.0.142:8080/stock/generate-report"
        );
        ...
        setData({
          totalRegisteredProducts: result.totalRegisteredProducts || 0,
    ...
    <SafeAreaView className="" style={{ flex: 1 }}>
      <View className="m-6" style={{ marginTop: statusBarHeight + 20 }}>
        <View className="flex-row justify-between">
          <Text className="text-2xl font-latoBold ">Olá, {username}</Text>
